import Image from "next/image";
import { FaLink } from "react-icons/fa";
import Link, { LinkProps } from "next/link";

interface Props {
	item: any;
	variant?: "rounded" | "circle";
	size?: "small" | "medium";
	effectActive?: boolean;
	href: LinkProps["href"];
}

export default function Card({
	item,
	variant = "circle",
	size = "small",
	effectActive = false,
	href,
}:Props) {
	const { name, image } = item ?? {};
	const imageSize: any =
		(size === "small" && 190) || (size === "medium" && 220);
	
	const placeholderImage = `/assets/placeholder/card-${size}.svg`;
	
	return (
		<Link
			href={href}
			className="group flex justify-center text-center flex-col"
		>
			<div
				className={`relative inline-flex mb-3.5 md:mb-4 lg:mb-5 xl:mb-6 mx-auto ${variant === "rounded" ? "rounded-md" : "rounded-full"
					}`}
			>
				<div className="flex">
					<Image
						src={image ? `${process.env.NEXT_PUBLIC_OPENCART_DOMAIN_URL}/image/${image}` : placeholderImage}
						alt={name || "Категория"}
						width={imageSize}
						height={imageSize}
						quality={100}
						className={`object-cover aspect-square bg-gray-300 ${variant === "rounded" ? "rounded-md" : "rounded-full"
							}`}
					/>
				</div>
				{effectActive === true && (
					<>
						<div
							className={`absolute top left bg-black w-full h-full opacity-0 transition-opacity duration-300 group-hover:opacity-30 ${variant === "rounded" ? "rounded-md" : "rounded-full"
								}`}
						/>
						<div className="absolute top left h-full w-full flex items-center justify-center">
							<FaLink className="text-white text-base sm:text-xl lg:text-2xl xl:text-3xl transform opacity-0 scale-0 transition-all duration-300 ease-in-out group-hover:opacity-100 group-hover:scale-100" />
						</div>
					</>
				)}
			</div>
			<h3 className="text-heading text-sm md:text-base xl:text-lg capitalize">{name}</h3>
		</Link>
	)
}